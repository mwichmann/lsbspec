...
<TITLE>Scope</TITLE>
<PARA>
The LSB-Scan module defines the scanning components found on an
...
the LSB-Scan Module Standard.  Such references may be normative or
informative; a reference to specification shall only be considered
normative if it is explicitly cited as such.  The LSB-Scan Module
